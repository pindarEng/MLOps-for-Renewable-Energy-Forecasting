...
mlflow.set_experiment("XGBoost Wind Generation Prediction-1 __ no lag features")
print("setup complete")
with mlflow.start_run(run_name=f"xgb_model_nolag-{ver}"):
    print("started run")

    mlflow.sklearn.pyfunc.log_model("xgb_model_scaler_batches_nolag",python_model=ModelWithScaler(model = best_xgb, scaler= scaler))
    print("logged model")
    ...
    model_info = mlflow.register_model(
        "runs:/{}/xgb_model_scaler_batches_nolag".format(mlflow.active_run().info.run_id),
        name="model_xgb_model_scaler_batches_nolag"
    )
