...
try:
    ...
    df_latest_batches['lag_1'] = df_latest_batches['DE_wind_generation_actual'].shift(1)
    df_latest_batches['lag_2'] = df_latest_batches['DE_wind_generation_actual'].shift(2)
    df_latest_batches['lag_3'] = df_latest_batches['DE_wind_generation_actual'].shift(3)
    df_latest_batches.dropna(inplace=True)
...
stats.to_json("training_feature_stats_xgb.json", indent=4)

mlflow.set_tracking_uri('http://mlflow:5000') #remember to change to mlflow:5000
mlflow.set_experiment("XGBoost Wind Generation Prediction-1")
print("setup complete")
with mlflow.start_run(run_name=f"xgb_model-{ver}"):
    print("started run")

    mlflow.sklearn.pyfunc.log_model("xgb_model_scaler_batches",python_model=ModelWithScaler(model = best_xgb, scaler= scaler))
    print("logged model")
    ...
    print("logged R2")
    mlflow.log_artifact("training_feature_stats_xgb.json")

    model_info = mlflow.register_model(
        "runs:/{}/xgb_model_scaler_batches".format(mlflow.active_run().info.run_id),
        name="model_xgb_model_scaler_batches"
    )
    print("model logged with mlflow")

os.remove("training_feature_stats_xgb.json")