...
mlflow.set_experiment("RandomForest Wind Generation Prediction-1 __ lag features")
print("setup complete")
with mlflow.start_run(run_name=f"randomforest_model-{ver}"):
    print("started run")

    mlflow.sklearn.pyfunc.log_model("randomforest_model_scaler_batches_lag",python_model=ModelWithScaler(model = model, scaler= scaler))
    print("logged model")
    ...
    model_info = mlflow.register_model(
        "runs:/{}/randomforest_model_scaler_batches_lag".format(mlflow.active_run().info.run_id),
        name="model_randomforest_model_scaler_batches_lag"
    )
