    ...
    for page in pages:
        if "Contents" in page:
            for obj in page['Contents']:
                # Basic check: assumes filename contains timestamp parsable info
                # Adapt this logic if your filenames are different
                if obj['LastModified'].replace(tzinfo=None) >= cutoff_date:
                     # Check if it's a CSV file saved by the consumer
...
if __name__ == "__main__":
    ...
    #save a local file also. just in case.
    df.to_csv(f"dataset/historical/latest_batches.csv", index=False)
